...
#import "MostUsedTableController.h"


@interface MostUsedTableController ()
...
@implementation MostUsedTableController
...
extern NSString *resultsString;

@synthesize tableView;
@synthesize joinedComponents;
...
- (void)viewDidLoad {
    ...
    {
        navigationalBarImage.frame = CGRectMake(-5, 45, self.view.frame.size.width+20, 50); //Image scaled
    }
    else if ([[UIScreen mainScreen] bounds].size.height == 667) //iPhone 6 size
    {
...
-(void)retrieveFromParse {
    //QUERY FOR GETTING EXERCISES
    
    PFQuery * browseAllQuery = [PFQuery queryWithClassName:@"Exercises"];
    [browseAllQuery orderByDescending:@"ExerciseName"];
    ...
    [arrayQuery findObjectsInBackgroundWithBlock:^(NSArray *results, NSError *error) {
        ...
        if (!error) {
            arrayResults = [[NSMutableArray alloc] initWithArray:results];
            NSLog(@"%@", [arrayResults objectAtIndex:5]);
        }
    }];
...
//        NSString *resultsString = [arrayResults objectAtIndex:4];
//        browseDetail.setRepsLabelString = [[NSString alloc] initWithFormat:@"%@", resultsString];
//  }
    
//    NSString *URLString = [[videoURL objectAtIndex:indexPath.row] objectForKey:@"VideoURL"];
...
- (TableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *ll= [[browseAllArray objectAtIndex:indexPath.row] objectForKey:@"ExerciseName"];
    NSLog(@"%@", ll);
    UILabel *cellTitle;
